...
function emailCabecalho($pSTitulo)
{
	...
	$vSTexto .= '<title>"DPM Educação"</title>';
	$vSTexto .= '</head>';
	...
	$vSTexto .= '<th colspan="4" align="left">';
	$vSTexto .= '<img src="https://dpmeducacao.com.br/imgs/email/email_topo.jpg">';
	$vSTexto .= '</th>';
	$vSTexto .= '<th colspan="2" align="left">><br /></span></th>';
	$vSTexto .= '</tr>';
	//$vSTexto .= '<th colspan="2" align="left"><a href="https://dpmeducacao.com.br">https://dpmeducacao.com.br</a><p>CANAIS DE COMUNICAÇÃO DA DPM EDUCAÇÃO:</p> <p>Telefone: <strong data-cms-node="7">' . cSTelefone1 . '</strong><br />WhatsApp: (51) 98041-5821 ou (51) 99661-2022<strong data-cms-node="7">' . cSTelefone2 . ' - ' . cSTelefone3 . '</p></strong>E-mail: cursos@dpmeducacao.com.br</span></th>';
...
function inscricaoFinalizada($dados, $pSFormaPgto, $vSTemContrato = '')
{
	...
	$vSTexto .= '</td>';
	$vSTexto .= '<td align="left" style="padding: 5px 20px;"><span>' . $dados['codcurso'] . " - " . ($dados['curso'])
	...
	//foreach ($dados['destinatarios'] as $destinatario) {
		$retorno[] = envioEmailInscricao(($dados["titulo"]), ($vSTexto), $dados["email"], $dados["emailAlternativo"], $vSTemContrato); 
	//}

	return $retorno;
}

function envioEmailInscricao($pSTitulo, $pSMensagem, $pSAddAddress, $pSAddCCAddress, $vSTemContrato = '')
{
	...
	//	$pSAddAddress = ($pSAddAddress != 'ADMIN') ? $pSAddAddress : CFGEMAILRECEBIMENTO;
		...
		$mail->Password = "Educ@c@o@2023";
		$mail->setFrom('inscricoescursos@dpmeducacao.com.br', 'DPM Educação');
		$mail->addAddress('inscricoescursos@dpmeducacao.com.br', 'DPM Educação');
	...
	//	else
		...
		if ($pSAddAddress != '')
			$mail->addCC($pSAddAddress);
		if ($pSAddCCAddress != '')
			$mail->addCC($pSAddCCAddress);
